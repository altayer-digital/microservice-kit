...
const debug = require('./logger')('microservice-kit:lib:queue');
...
class Queue {
  ...
  sendEvent(eventName, opt_payload, opt_options) {
    if (!_.isString(eventName)) { return Promise.reject(new Error('Cannot send event to queue. Event name is required.')); }
    ...
    rv.progress = (callback) => {
      ...
      if (rpcCb_) { rpcCb_.progress = callback; }
